...
/** Currently using a dummy reader designed to be as simple as possible for easy
 * changes **/
class TimelineReader {
   public:
    TimelineReader(const char* filename) {
        for (auto&& file :
            ...
            std::vector<std::string_view> columns = {
                "patient_id", "start",      "end", "code",
                "value",      "event_type", "id",  "parent_id"};
            ...
            auto add_patient = [&]() {
                if (found_birth) {
                    ...
                    std::cout << "Warning, could not find birth for "
                              << patient.patient_id << std::endl;
                }
            };

            csv_iterator(path.c_str(), columns, ',', {},
    ...
    absl::Span<const uint32_t> get_patient_ids() const { return patient_ids; }

    Patient get_patient(uint32_t patient_id) { return patients[patient_id]; }
...
void convert_patients_to_extract(std::string patient_directory,
                                 std::string extract_file, int num_threads) {
    /*
        Currently we do a nullopt extractor and use the raw gzip files as
       format. Could obviously be optimized as necessary.
    */
...
void register_datasets_extension(py::module& root) {
    ...
    m.def("convert_patient_collection_to_patient_database",
          convert_patients_to_extract);

    py::class_<TimelineReader>(m, "PatientDatabase")
