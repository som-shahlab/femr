...
inline bool should_make_prediction(float last_prediction_age, float current_age,
                                   float next_age, int current_year) {
...
struct SurvivalCalculator {
    FlatMap<std::vector<float>> future_times;
        ...
        for (const Event& event : p.events) {
            if (event.value_type == ValueType::NONE) {
                for (const auto& parent : get_all_parents(event.code)) {
                    std::vector<float>* item = future_times.find_or_insert(
                        parent, std::vector<float>());
    ...
    std::pair<float, std::vector<std::pair<float, uint32_t>>>
    get_times_for_event(float event_age) {
        std::vector<std::pair<float, uint32_t>> events;
