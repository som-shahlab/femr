...
import piton.models.dataloader
...
@functools.partial(jax.jit)
def compute_grad(beta, reprs, log_times, is_events, l=0):
    hazards = jnp.dot(reprs, beta)
...
@functools.partial(jax.jit)
def compute_hessian(beta, u, reprs, log_times, is_events, l=0):
    hazards = jnp.dot(reprs, beta)
...
@functools.partial(jax.jit, static_argnames=("gradient", "hessian"), donate_argnums=(0, 1, 2))
def conjugate_gradient(last_w, last_gradient, last_u, gradient, hessian, l):
    g = gradient(last_w, l=l)
    if last_gradient is None:
        ...
        u = g - last_u * beta
    w = last_w - (jnp.dot(g, u) / hessian(last_w, u, l=l)) * u
    return w, g, u
...
for frac in [1]:
    ...
    partial_grad = functools.partial(
        compute_grad,
        reprs=train_reprs,
        log_times=train_log_times,
        is_events=train_is_events,
    )
    partial_hessian = functools.partial(
        compute_hessian,
        reprs=train_reprs,
        log_times=train_log_times,
        is_events=train_is_events,
    ...
    for l_exp in np.linspace(end_l, start_l, num=20):
        ...
        while True:
            ...
            beta, g, u = conjugate_gradient(beta, g, u, partial_grad, partial_hessian, l)
        ...
        def get_c(split_index):
            ...
            return piton.extension.metrics.compute_c_statistic(
                event_time,
                is_censor,
                time_bins[:-1],
                hazards[mask, :],
