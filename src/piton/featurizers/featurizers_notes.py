...
class NoteFeaturizer:    
    ...
    def embed_parallel(self, args: Tuple) -> List[NotesEmbedded]:
        ...
        for patient_ids in patient_ids_chunks:
            ...
            if device != 'cpu':
                ...
                print_log("embed", f"chunk #{chunk_id} | device {device}, total (mem): {t}, reserved: {r}, allocated: {a}, free: {t - r - a}")
            ...
            print_log("embed", f"chunk #{chunk_id} | device {device} | # of batches of size {batch_size}: {len(train_loader)}")
    ...
    def featurize(
        ...
        print_log("featurize", f"Using {num_chunks} chunks, with {num_patients_per_chunk} patients per chunk")
        ...
        save_to_pkl(
            result_tuple, 
            os.path.join(self.path_to_output_dir, f"features.pkl")
        )
