...
def map_omop_concept_ids_to_piton_codes(ontology: extension_datasets.Ontology, omop_concept_ids: List[int]) -> List[int]:
    codes: List[int] = []
...
class CodeLabeler(TimeHorizonEventLabeler):
    ...
    def __init__(
        ...
        Args:
            ...
            prediction_codes (List[int]): The codes for events at which you make predictions.
...
class OMOPConceptCodeLabeler(CodeLabeler):
    ...
    orignal_omop_concept_ids: List[int] = []
    omop_concept_ids: List[int] = []

    def __init__(
        self, ontology: extension_datasets.Ontology, time_horizon: TimeHorizon, prediction_codes: List[int] 
    ):
        codes: List[int] = map_omop_concept_ids_to_piton_codes(ontology, self.omop_concept_ids)
        prediction_codes: List[int] = [] # TODO
        super().__init__(
            codes=codes,
...
class MortalityCodeLabeler(CodeLabeler):
    ...
    def __init__(
        ...
        admission_code: int = ontology.get_dictionary().index(INPATIENT_VISIT_CODE)
        
        DEATH_CODE = "Condition Type/OMOP4822053"
        death_code: int = ontology.get_dictionary().index(DEATH_CODE)
...
class LupusDiseaseCodeLabeler(CodeLabeler):
    ...
    def __init__(self, ontology: extension_datasets.Ontology):
        ...
        for code in icd9_codes:
            codes |= _get_all_children(ontology, dictionary.index("ICD9CM/" + code))
        for code in icd10_codes:
            codes |= _get_all_children(ontology, dictionary.index("ICD10CM/" + code))
...
class HighHbA1cCodeLabeler(Labeler):
    ...
    def __init__(
        self, ontology: extension_datasets.Ontology, last_trigger_days: int = 180,
    ):
...
class HypoglycemiaCodeLabeler(OMOPConceptCodeLabeler):
    ...
    orignal_omop_concept_ids = [
        380688, 4226798, 36714116, 24609, 4029423, 45757363, 4096804, 4048805,
...
class HyperkalemiaCodeLabeler(OMOPConceptCodeLabeler):
    ...
    orignal_omop_concept_ids = [ 434610]
    omop_concept_ids = [
        434610, 4030355, 4185833, 4183002, 4028948, 4029592, 4201725, 4029591, 
        4071744, 4236458, 4146120,
    ]
...
class HyponatremiaCodeLabeler(OMOPConceptCodeLabeler):
    ...
    omop_concept_ids: List[int] = [
        435515, 4232311, 4029590, 4028947, 4225276, 4227093, 4252414, 4164126, 
        4175012, 4177324, 4048926, 4253214, 4139394, 4215499, 4028946,
    ]
...
class NeutropeniaCodeLabeler(OMOPConceptCodeLabeler):
    ...
    omop_concept_ids = [
        37117238, 37205095, 36716754, 36716753, 4095623, 320073, 
        42596532, 42536512, 42539535, 4101126,
    ]
