...
ehr_ml/ehr_ml/labeler.py /
@Lalaland
Lalaland Run formatter + fix pyproject
Latest commit 245dd34 on May 7, 2021
 History
 1 contributor
1648 lines (1273 sloc)  51.8 KB
...
class Label:
    ...
    def __repr__(self) -> str:
        if self.label_type == "binary":
            return f"Label(day_index={self.day_index}, is_positive={self.is_positive})"
        elif self.label_type == "numeric":
            return f"Label(day_index={self.day_index}, numeric_value={self.numeric_value})"
        elif self.label_type == "survival":
            return f"Label(day_index={self.day_index}, survival_value={self.survival_value})"
        elif self.label_type == "categorical":
            return f"Label(day_index={self.day_index}, categorical_value={self.categorical_value})"
        else:
            raise ValueError(f"Invalid label type of {self.label_type}")

    def to_dict(self) -> Dict[str, Any]:
        result: Dict[str, Any] = {"day_index": self.day_index}

        if self.label_type == "binary":
            result["is_positive"] = self.is_positive
        elif self.label_type == "numeric":
    ...
    @classmethod
    def from_dict(cls, stored: Mapping[str, Any]) -> Label:
        survival_value: Optional[SurvivalValue]
        ...
        return Label(
            stored["day_index"],
            is_positive=stored.get("is_positive"),
...
class Labeler(ABC):
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        """
        Label the provided patient.
        Args:
            patient: A patient timeline struct
        Returns:
            A list of :class:`Label` containing every label for the patient
        """
    ...
    def get_possible_patient_ids(self) -> Mapping[bool, Optional[Set[int]]]:
        ...
        Returns:
            None if the patient ids are not known otherwise a map where each label maps to the set of patient
            ids for that label.
        """
...
class FixedTimeHorizonEventLabeler(Labeler):
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        for i in self.get_prediction_days(patient):
            ...
            if event_soon:
                results.append(Label(day_index=i, is_positive=True))
            elif evidence_of_alive:
                results.append(Label(day_index=i, is_positive=False))
...
class InfiniteTimeHorizonEventLabeler(Labeler):
    ...
    @abstractmethod
    def get_first_event_age(self, patient: timeline.Patient) -> Optional[int]:
        """
...
class YearHistoryRequiredLabeler(Labeler):
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        return [
            label
            for label in labels
            if patient.days[label.day_index].age >= (first_age + 365)
        ]
            ...
            for label in labels:
                ...
                if self.labeler_type == "binary":
                    self.possible_patient_ids[
                        cast(bool, actual_label.is_positive)
                    ].add(patient_id)

    def label(
        self, patient: timeline.Patient, patient_id: Optional[int] = None
    ) -> List[Label]:
    ...
    def save(
        ...
        if force_prevalence is not None:
            ...
            if labeler.get_labeler_type() != "binary":
                raise ValueError(
                    f"Can only force a prevelence on a binary labeler, not {labeler.get_labeler_type()}"
                )
        ...
        for patient_id in timelines.get_patient_ids():
            if (
                ...
                for label in generated_labels:
                    if label.day_index < 0 or label.day_index >= len(
                        patient.days
                    ):
                        raise ValueError(
                            f"The labeler {labeler} produced a label with invalid day index {label.day_index}"
                        )
                ...
                if len(generated_labels) > 0:
                    ...
                    if force_prevalence is not None:
                        for i, label in enumerate(generated_labels):
                            label_indices[cast(bool, label.is_positive)].append(
                                (patient_id, i)
                            )
...
class PredictionAfterDateLabeler(Labeler):
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        for label in self.sublabeler.label(patient):
            if patient.days[label.day_index].date >= self.start_date:
                result.append(label)
...
class OlderThanAgeLabeler(Labeler):
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        return [
            label
            for label in labels
            if patient.days[label.day_index].age >= self.age_cutoff_in_days
        ]
            ...
            for observation_with_value in day.observations_with_values:
                if (
                    ...
                    results.append(
                        Label(
                            day_index=i - 1,
                            is_positive=observation_with_value.numeric_value
                            > self.greater_than,
                        )
                    )
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        for i, day in enumerate(patient.days):
            ...
            if day.age == current_admission.start_age:
                ...
                labels.append(Label(day_index=i, is_positive=is_male))
    ...
    def label(self, patient: timeline.Patient) -> List[Label]:
        ...
        for i, day in enumerate(patient.days):
            ...
            if day.age == current_admission.start_age:
                ...
                if i != 0:
                    labels.append(
                        Label(day_index=i - 1, is_positive=died_in_admission)
                    )
                ...
                for obsValue in day.observations_with_values:
                    if (
                        ...
                        return [
                            Label(
                                day_index=i - 1,
                                is_positive=obsValue.text_value
                ...
                if (
                    ...
                    if last_trigger is None or (day.age - last_trigger) > 180:
                        if had_previous_negative_test:
                            results.append(
                                Label(day_index=i - 1, is_positive=is_diabetes)
                            )
                ...
                if code in self.za_drug_ids or code in self.denosumab_drug_ids:
                    if i == 0 or not has_cancer:
                        ...
                        return [
                            Label(
                                day_index=i - 1,
                                is_positive=code in self.za_drug_ids,
                            )
                        ]
                ...
                if code in self.penicillin or code in self.cephalosporin:
                    if i == 0 or not has_cancer:
                        ...
                        return [
                            Label(
                                day_index=i - 1,
                                is_positive=code in self.penicillin,
                            )
                        ]
            ...
            for code in day.observations:
                ...
                if code in self.pd1 or code in self.ana:
                    if i == 0 or not has_cancer:
                        ...
                        return [
                            Label(day_index=i - 1, is_positive=code in self.pd1)
                        ]
