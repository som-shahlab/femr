...
class LongInpatientAdmissionLabeler(Labeler):
    ...
    def __init__(
        self, ontology: extension_datasets.Ontology, long_time: datetime.timedelta,
    ):
        dictionary = ontology.get_dictionary()
        self.admission_codes: List[int] = [ dictionary.index(x) for x in get_inpatient_admission_concepts() ]
    ...
    def label(self, patient: Patient) -> List[Label]:
        ...
        for event in patient.events:
            if event.code in self.admission_codes:
                is_long_admission: bool = (event.end - event.start) >= self.long_time
