...
def test_constructor():
    ...
    labeler = DummyThrombocytopeniaLabeler(ontology, time_horizon, 'severe')
    assert labeler.get_time_horizon() == time_horizon
    assert labeler.severity == 'severe'
    assert set(labeler.outcome_codes) == set([3, 4, 7, 8, 12, 13,])
...
def test_labeling():
    ...
    assert labeler.get_outcome_times(patient) == true_outcome_times, f"{labeler.get_outcome_times(patient)}"
